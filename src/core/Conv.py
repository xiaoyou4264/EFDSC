...
class ConvAE:
    ...
    def __init__(self,x,params):
        ...
        # 对y进行傅里叶层的操作
        # FFT_layer层
        fft_layer_set = [{'type': 'fft_layer', 'name': 'fft_layer', 'output_num': params['n_clusters']}]
        outputs = stack_layers(y, fft_layer_set)

        # layers = [
        #           {'type': 'Orthonorm', 'name':'orthonorm'}
        #           ]
        #
        # # 调用layer.py文件中的stack_layers方法，就是一个正交化层，将y作为输入，outputs为输出
        ...
        loss_SPNet = (K.sum(W * Dy))/params['batch_size']
